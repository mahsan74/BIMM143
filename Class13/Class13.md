...
we will need to remove that odd first column
in `countData` namely `contData$length`:
...
```
    ...
    Info: Working in directory /Users/mahsa/Desktop/BioInformatics/Class13
...
Cell cycle, mitotic has the most significant Entities p-value. The KEGG
result is different from the Reactome result and factors like different
database content could be a reason for different result. Another factor
coul be using different sets of genes and identifiers.

# Section 5. GO online(optional)

performing Gene Set GO Enrichment online to determine over-represented
or under- represented GO terms for a given set of genes:

**Q9:** What pathway has the most significant “Entities p-value”? Do the
most significant pathways listed match your previous KEGG results? What
factors could cause differences between the two methods?

the most significant is PRA1 Family protein 2 which is also different
from KEGG result due to using different dataset.
